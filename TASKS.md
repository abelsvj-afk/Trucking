...
| 1.5 | Write the RLS tenant-isolation test (`docs/design/testing.md`) | 1.4 | Two-company test proves company A can never read/write company B's rows, for every table | ✅ Written (`tests/integration/rls-tenant-isolation.test.ts`); RLS itself verified clean via Supabase's own security advisor (0 findings after a hardening fix — see below). Running the actual committed test file end-to-end is still pending `SUPABASE_SERVICE_ROLE_KEY`. |
