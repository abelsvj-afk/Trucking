...
| 3.9 | Maintenance events: backend + Fleet screens | 3.1 | Shows in the relevant truck's maintenance history | ✅ Done. `description`/`service_date`/`truck_id` required; `cost_cents`/`mileage_at_service` nullable non-negative. Satisfies "shows in the relevant truck's maintenance history" by adding a truck filter to the list screen, reusing the same `filterableFields` mechanism loads uses for `?status=` (declares `["truck_id"]`) rather than building a new truck-detail page that nothing else needed yet. Lives under Fleet, not Money, since it's about a truck's service record, not cash flow. Verified: `tsc --noEmit`, `eslint .`, `vitest run` (61/61), `next build` (`/fleet/maintenance-events`, `/fleet/maintenance-events/new`, both `/api/v1/maintenance-events` routes all present). |
