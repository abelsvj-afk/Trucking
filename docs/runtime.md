...
`docs/automation.md` established *that* this job runs on a schedule; this is the concrete answer to *how* — **changed** from the original Vercel Cron Job design now that the runtime is a persistent process rather than serverless. Fly.io has no built-in cron-job primitive equivalent to Vercel's, but a persistent process doesn't need one: an **in-process scheduler**, started once when the server boots (`setInterval`, or a small scheduling library if the interval logic outgrows that — not decided further than this until it's actually built), calls the job logic in `services/ai`/`services/integrations` directly, in-process, on the interval `docs/automation.md` specifies.
