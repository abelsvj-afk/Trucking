...
**Model:** the Anthropic Claude API, called server-side only (never from the browser), API key handled exactly like any other secret in `docs/design/security.md`. This is the natural choice given the project itself is being built with Claude — no separate AI-provider evaluation needed to justify it, though the specific model (e.g. a mid-tier model balancing cost and quality for structured business recommendations) is picked per capability based on what that capability actually needs, not fixed globally.
...
- **Context:** pulls from `services/integrations` (fuel-market data, regulatory/news sources — specific sources still undecided, per `docs/requirements.md`) — external, untrusted content, never treated as instructions.
