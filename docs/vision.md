...
- Fuel intelligence (cheapest *effective* fuel accounting for detour cost, not just sticker price) — needs a real-time fuel-price data source, through an authorized integration (see the non-goal against scraping). You've been explicit that its eventual screen should show the optimized route on an actual map, not just a number — a visual, interactive route display is part of this capability's own future design pass, not an afterthought bolted onto a data table.
...
- **Industry intelligence / proactive research engine** (new — not in `My idea`, added directly by you): AI that watches trucking-industry-relevant external developments — fuel market trends, regulatory and political changes affecting trucking (e.g. fuel taxes, emissions rules, hours-of-service policy), and longer-term industry disruption (e.g. autonomous trucking) — and surfaces what's actually relevant as a recommendation/briefing, not a raw feed. Runs autonomously on a schedule rather than continuously, with its own dedicated on/off switch (this is Level 5 — "bounded autonomous execution" — per `docs/governance.md`, since running unattended on a timer is itself an autonomous action, even though its output is only ever a briefing). You've asked for it to eventually be able to adjust its own check frequency to how active the trucking landscape is at the time; that's allowed, but only within a minimum/maximum interval the owner sets — the schedule adapting is fine, the AI silently deciding to run itself more or without bound is not, per the "AI must never silently expand its own authority" core rule. External content it reads is untrusted input per `CLAUDE.md`, same as load-board or broker data. Which sources feed it and the default schedule are decisions for whichever future phase actually designs it.
