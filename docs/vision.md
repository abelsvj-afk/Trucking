...
- **Industry intelligence / proactive research engine** (new — not in `My idea`, added directly by you): AI that continuously watches trucking-industry-relevant external developments — fuel market trends, regulatory and political changes affecting trucking (e.g. fuel taxes, emissions rules, hours-of-service policy), and longer-term industry disruption (e.g. autonomous trucking) — and surfaces what's actually relevant as a recommendation/briefing, not a raw feed. Like everything else here, it only ever recommends (per `docs/governance.md`'s Level 1/2 boundary); external content it reads is untrusted input per `CLAUDE.md`, same as load-board or broker data. Exactly how proactive it is (push alerts vs. an on-demand digest) and which sources feed it are decisions for whichever future phase actually designs it, not fixed here.
